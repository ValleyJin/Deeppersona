...
### Q. (슬라이드 관련) "대화 3천건"은 무슨 대화이고, "8천 노드"의 노드는 무엇인가? 8천 개 속성을 만든다는 뜻인가?

**대화 출처 (논문 §3.1, 확정)** — "3천건"은 축약이었고 실제는 3개 소스:

| 출처 | 수 | 정체 |
|---|---|---|
| Puffin (`LDJnr/Puffin`) | 3,000 대화 | 실제 사람↔챗봇 대화 |
| prefeval_implicit_persona | 1,000 대화 | 선호/페르소나가 은연중 드러나는 대화 |
| Llama-3.2-3B-HiCUPID | 60,000 샘플 | 인간-챗봇 상호작용 샘플 |

→ 합 ~6.4만 대화에서 GPT-4.1이 QA를 *Non/Partially/Personalizable*로 분류 → **62,224개 개인화 QA 쌍**이 트리 원재료. 성격: **자기노출(self-disclosure) 발화**에서 속성 채굴.

**노드의 정의 (확정)**:
- 노드 = 트리의 **모든 계층**에 있는 속성 항목 하나 (top 대분류 + 중간 소분류 + 잎).
- 논문: 병합 후 **8,496 노드**(전 계층 합계). README "8,000+"가 이것.
- **값을 채우는 잎(leaf) 슬롯은 이 fork에서 2,297개**뿐. "8천"에는 `Career`·`Background` 같은 폴더 성격 중간 노드가 다 포함됨.
- 비유: 폴더+파일 다 세면 8,496, 실제 "파일"(값 칸)은 2,297.

→ 슬라이드 반영: 모듈 01 파이프라인 도식(`실사용 대화/자기노출 QA`, `8,496 노드` + srcnote), 모듈 02 추출 프레임·핵심 숫자에 출처·노드 정의 명시.
...
| 항목(임베딩 한정) | 캐시 없음 | 캐시 (pickle) |
|---|---|---|
| 페르소나 1개당 슬롯 임베딩 호출 | 2,297번 | 0번 (dict lookup) |
| 페르소나 1개당 시드 임베딩 호출 | 1번 | 1번 |
| 10만 명 슬롯 임베딩 총 호출 | 약 2.3억 | 0 |
| 10만 명 **임베딩** 비용(추정) | ≈$300 | ≈$1.5 |

**(중요) 위 비용은 임베딩(ada-002)만이다.** ada-002 가격 ≈$0.10/1M tokens, 슬롯 경로 ≈12토큰 기준:
- 캐시 없음: 2,297×~12토큰 ≈ 2.8만 토큰/persona → 10만 명 ≈ 2.8e9 토큰 ≈ **$300**.
- 캐시: 시드 임베딩만(~150토큰)×10만 ≈ 1.5e7 토큰 ≈ **$1.5**.
- 이전 판(§ 초안)의 "$23,000/$10"은 ~100× 과대치였고, "생성 비용"이라 라벨한 것도 오류. **실제 값 생성(GPT-4o LLM 호출) 비용은 캐시와 무관하게 동일**하며, 그게 총비용의 대부분이다. pkl은 오직 임베딩 비용만 없앤다.

비대칭의 원리:
- **슬롯 임베딩**: 2,297개 텍스트가 고정 → 한 번 만들어두면 영원히 재사용
- **시드 임베딩**: 페르소나마다 다른 사람 → 매번 새로 계산 (캐시 불가)

알고리즘 자체는 pickle 없이도 동일하게 작동. 단지 매번 슬롯 임베딩 API 호출만 더 들어갈 뿐(값 생성 LLM 비용은 그대로).
