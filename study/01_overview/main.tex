...
\begin{frame}{두 단계로 나눈다: 트리를 짓고 $\to$ 트리를 걷는다}
  ...
  \begin{tikzpicture}[font=\footnotesize,>=Latex,
    ...
    \node[box] (dlg) at (0,1.3) {ChatGPT\\대화 3천건};
    \node[box,right=0.7cm of dlg] (ext) {속성 구절\\추출};
    \node[box,right=0.7cm of ext] (mrg) {계층 병합\\+ 검증};
    \node[box,right=0.7cm of mrg] (tree) {\textbf{속성 트리}\\{\scriptsize 8천 노드}};
  ...
  \end{tikzpicture}
  \end{center}
  \srcnote{논문 Fig.\ 2의 흐름을 코드 관점으로 재구성. 점선 = 만들어 둔 트리를 매 페르소나가 재사용.}
\end{frame}
